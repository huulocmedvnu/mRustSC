...
class LinearAlgebraOps(Protocol):
    ...
    def solve(self, matrices: Array, vectors: Array) -> Array:
        """Solve ``matrices @ x == vectors``, following NumPy's convention: a
        right-hand side with one axis fewer than ``matrices`` is a (stack of)
        vector(s), otherwise a matrix. Batched over the leading axis."""
        ...
