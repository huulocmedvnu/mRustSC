...
# `pp.neighbors`). `scanpy` appears exactly twice and never for arithmetic:
#
# * to **load** the dataset (`sc.datasets.pbmc3k`), and
# * to **plot** the results (`sc.pl.*`), because plotting is scanpy's job and the results
#   land in the AnnData slots `sc.pl` reads.
#
# There is not a single `sc.pp.*` or `sc.tl.*` call below — that is enforced by an AST
# check in `_assert_no_scanpy_compute()` at the bottom of this file, which the script runs
# on itself before it does any work.
#
# Run it as a script:
#
# ```bash
...
# ```
#
# The cells use the jupytext "percent" format (`# %%`), so `jupytext --to notebook
# docs/tutorials/pbmc3k_clustering.py` produces the `.ipynb` when jupytext is installed.
...
def main() -> int:
    ...
    # **log-normalised** matrix (recovered from `adata.raw`), not the scaled one, because
    # fold changes on z-scored data are meaningless — this mirrors the scanpy tutorial.
...
# ## Compliance check
#
# The script parses its own source and fails if any `sc.pp.*` or `sc.tl.*` attribute
# appears — the guarantee that no computation touched scanpy. It runs before `main()`.
